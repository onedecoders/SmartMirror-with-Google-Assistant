...
class SystemBrowser(object):
    ...
    def display(self, html):
        ...
        #subprocess.call("xdotool key F11", shell=True)
    ...
    def startup(self):
        ...
        webbrowser.open(self.filename, autoraise=True)
        #subprocess.call("xdotool key F11", shell=True)
        #print(subprocess.check_output(namess).decode("utf-8"))
        
    def switchTab(self, windowName,delayTime):
        
        if (windowName.find("Chromium") != -1):
            time.sleep(delayTime)
            subprocess.call("wmctrl -a MagicMirror",shell=True)
        elif (windowName.find("MagicMirror") == -1):
            subprocess.call("wmctrl -a MagicMirror",shell=True)
...
class Myassistant():
    ...
    def process_event(self,event):
        ...
        if event.type == EventType.ON_START_FINISHED:
            ...
            # print("Fullscreen starting now")
            # subprocess.call("wmctrl -a Chromium -b fullscreen,maximized_vert",shell=True)
            # print("FS started")
            #system_browser.startup()

        if event.type == EventType.ON_CONVERSATION_TURN_STARTED:
            global isOpenBrowser
            if (isOpenBrowser == False):
                system_browser.startup()
            isOpenBrowser = True
        ...
        if (event.type == EventType.ON_CONVERSATION_TURN_TIMEOUT or event.type == EventType.ON_NO_RESPONSE):
            self.can_start_conversation = True
            namess ="xdotool", "getwindowfocus", "getwindowname"
            windowName = subprocess.check_output(namess).decode("utf-8")
            system_browser.switchTab(windowName,0)
        ...
        if (event.type == EventType.ON_CONVERSATION_TURN_FINISHED and
            ...
            namess ="xdotool", "getwindowfocus", "getwindowname"
            windowName = subprocess.check_output(namess).decode("utf-8")
            system_browser.switchTab(windowName, 5)
